  ...
  Class.prototype.events = function(){
    ...
    _WIN.on('resize', function(){ //自适应
      if (that.autoCol) {
        ...
        var autoCol = that.autoCol;
        // 存在自适应的列
        layui.each(config.cols, function (i, cols) {
          layui.each(cols, function (j, col) {
            if (col.field && (autoCol[col.field]||autoCol[col.field]===0)) {
              col.width = autoCol[col.field].width;
            }
          })
        });
        // 调整宽度
        that.setArea();
        layui.each(that.autoCol, function (colField, col) {
          var widthNew = 0;
          layui.each(config.cols, function (i, cols) {
            layui.each(cols, function (j, col) {
              // 和并列的不需要处理
              if(col.colspan > 1 || (col.colspan === 1 && !col.field)) return;
              if (col.field === colField || j === colField) {
                widthNew = col.width;
                return true;
              }
            })
          });
          that.getCssRule(colField, function (item) {
            item.style.width = parseFloat(widthNew) + 'px';
          })
        });
      }
      that.fullSize();
